...
import { CobrarCreditoResponse } from "@/services/dto/pedidos/cobrarCredito";
...
export const apiServiceCreditos = {
  ...
  cobrar: async ({ token, forma_pago_id, monto, id }: { token: string | null; forma_pago_id: number; monto: number, id: number }) => {
    try {
      const { data, status } = await BASE.post(`/creditos/cobrar/${id}`, { forma_pago_id, monto }, { headers: { Authorization: token } });
      return new CobrarCreditoResponse({ success: data.success as boolean, status, results: data.results, message: "" });
    } catch (e) {
      if (isAxiosError(e)) {
        throw new Error(e.response?.data.message || "Error en conexion");
      }
